...
export async function generate(key, body, { signal, onStatus = () => {}, fetcher = fetch, interval = 1800, timeout = 180000 } = {}) {
    const boundedSignal = signal ? AbortSignal.any([signal, AbortSignal.timeout(timeout)]) : AbortSignal.timeout(timeout);
    ...
    while (true) {
        ...
        await new Promise((resolve, reject) => {
            const abort = () => { clearTimeout(timer); reject(boundedSignal.reason); };
