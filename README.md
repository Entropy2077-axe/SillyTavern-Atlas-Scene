...
- API Key 支持保存和清除，保存到当前酒馆用户设置，刷新后自动恢复。该字段为明文，分享或备份设置时请注意。
- 从 Atlas 公开 `/api/v1/models` 目录拉取模型，只列出 Image / TEXT-TO-IMAGE，支持手动输入 ID。其他模型暂用服务端默认参数，尺寸设置仅用于 z-image/turbo。
- 移除固定角色外观，自动提取当前角色卡（群聊读取成员卡）及本轮激活世界书。提炼时明确启用酒馆世界书流程 `skipWIAN: false`，使用其原生关键词、常驻、递归和预算规则。
- 只使用实际激活的条目，不强行读取整本世界书。显式补充材料上限为角色卡 16000 字符、世界书 24000 字符；提炼流程还会获得酒馆原生注入的上下文。
- 切换聊天清空激活快照；立即绘图模式使用上一轮快照，刷新后的首次立即绘图没有缓存，因此世界书角色卡推荐“回复完成后 + 提炼”。
...
4. 填入自己的 Atlas Key，点击“保存 Key”和“查询余额”验证，然后启用“自动绘图”。点击“保存 Key”后刷新会自动恢复；发布包不包含测试 Key。
...
- 可配置模型、尺寸、上下文消息数、画面风格。其他模型必须接受同样的 Atlas 文生图参数；仅 z-image/turbo 已实测；其他模型使用服务端默认参数，若该模型要求额外必填字段，接口会报错。
...
场景文本或提炼后的提示词会发给 Atlas；启用提示词提炼时，当前酒馆聊天模型也会收到场景材料并产生一次额外文本请求。风格是全局设置，角色外观来自角色卡和实际激活的世界书。不要在准备发送的场景材料中放入密钥。

Key 使用标准 Bearer 认证，仅发往固定域名 `api.atlascloud.ai`，点击保存后写入酒馆用户设置，不写入聊天、源码或日志。余额按官方接口的 `available.value` 与 `available.currency` 展示。
